...
subroutine private_sneut5(temp,den,abar,zbar,snu)
...
!c..local variables
      ...
      double precision t9,xl,xldt,xlp5,xl2,xl3,xl4,xl5,xl6,xl7,xl8,xl9, &
                       ...
                       a0,a1,a2,a3,b1,b2,b3,c00,c01,c02,c03,c04,c05,c06,&
                       ...
                       rp1,rn1,dum,dumdt,dumdd,dumda,dumdz,             &
...
!c..brem
      double precision t8,t812,t832,t82,t83,t85,t86,t8m1,t8m2,t8m3,t8m5,t8m6,    &
...
!c..numerical constants
      ...
      parameter        (fac1   = 5.0d0 * (2.0D0*acos(0.0D0)) / 3.0d0,   &
                        fac2   = 10.0d0 * (2.0D0*acos(0.0D0)), 	 	&
                        fac3   = (2.0D0*acos(0.0D0)) / 5.0d0, 	        &
                        oneth  = 1.0d0/3.0d0,				&
                        twoth  = 2.0d0/3.0d0,				&
                        con1   = 1.0d0/5.9302d0,			&
                        sixth  = 1.0d0/6.0d0,				&
...
!c..propagate through the routine. cv and ca are the vector and axial currents.
      ...
      double precision theta,xnufam,cv,ca,cvp,cap,tfac1,tfac2,tfac3,tfac4,tfac5,tfac6
...
!c..equation 3.7, compute the expensive trig functions only one time
      ...
      a0 = 0.5d0*c00 + c01*cos1 + dd01*sin1 + c02*cos2 + dd02*sin2 &
      ...
      a1 = 0.5d0*c10 + c11*cos1 + dd11*sin1 + c12*cos2 + dd12*sin2 &
      ...
      a2 = 0.5d0*c20 + c21*cos1 + dd21*sin1 + c22*cos2 + dd22*sin2 &
...
!c..equation 3.4
      ...
      dum    = 3.0d0*zeta2
      xdendt = dum*zetadt - xldt*(6.290d-3*xlm2 + 2.0d0*7.483d-3*xlm3 + 3.0d0*3.061d-4*xlm4)
      xdendd = dum*zetadd
      xdenda = dum*zetada
      xdendz = dum*zetadz
...
!c..equation 3.3
      ...
      dumdt  = xldt*(1.875d8 + 2.0d0*1.653d8*xl + 3.0d0*8.449d8*xl2 - 4.0d0*1.604d8*xl3)
...
!c..equation 5.21
       fb =  0.5d0 * 0.17946d0  + 0.00945d0*u + 0.34529d0 - 0.05821d0*cos1 - 0.04969d0*sin1  &
             - 0.01089d0*cos2 - 0.01584d0*sin2 - 0.01147d0*cos3 - 0.00504d0*sin3	     &
             - 0.00656d0*cos4 - 0.00281d0*sin4 - 0.00519d0*cos5 

       c00 =  a0*(0.00945d0 + 0.05821d0*sin1  - 0.04969d0*cos1 &
...
!c..equation 5.22
       ...
       c01 = a0*(-0.02342d0  + 0.00944d0*sin1 - 0.02213d0*cos1  &
...
!c..equation 5.23
       gb =  0.5d0 * 0.00766d0 - 0.01259d0*u + 0.07917d0   &
             ...
             + 0.00044d0*cos4 - 0.00089d0*sin4 + 0.00158d0*cos5

       c02 = a0*(-0.01259d0 + 0.00710d0*sin1 + 0.02300d0*cos1   &
...
!c..equation 5.24
       gt =  -0.5d0 * 0.00769d0  - 0.00829d0*u + 0.05211d0  &
             ...
             + 0.00031d0*cos4 - 0.00018d0*sin4  + 0.00069d0*cos5 

       c03 = a0*(-0.00829d0 - 0.00356d0*sin1 + 0.01052d0*cos1 &
...
!c..declare
      ...
      double precision f,an,a1(12),b1(12),a2(12),b2(12),rn,den,ff,z,drn


!c..load the coefficients of the expansion
      data  an,m1,k1,m2,k2 /0.5d0, 4, 3, 6, 5/
      data  (a1(i),i=1,5)/ 1.999266880833d4,   5.702479099336d3, 6.610132843877d2,   3.818838129486d1, 1.0d0/
      data  (b1(i),i=1,4)/ 1.771804140488d4,  -2.014785161019d3, 9.130355392717d1,  -1.670718177489d0/
      data  (a2(i),i=1,7)/-1.277060388085d-2,  7.187946804945d-2, -4.262314235106d-1,  4.997559426872d-1, &
                         -1.285579118012d0,  -3.930805454272d-1, 1.0d0/
      data  (b2(i),i=1,6)/-9.745794806288d-3,  5.485432756838d-2, -3.299466243260d-1,  4.077841975923d-1, &
...
!c..load the coefficients of the expansion
      ...
      data  (a1(i),i=1,8)/ 1.71446374704454d7,    3.88148302324068d7, 3.16743385304962d7,    1.14587609192151d7, &
                           1.83696370756153d6,    1.14980998186874d5, 1.98276889924768d3,    1.0d0/
      data  (b1(i),i=1,8)/ 9.67282587452899d6,    2.87386436731785d7, 3.26070130734158d7,    1.77657027846367d7, &
                           4.81648022267831d6,    6.13709569333207d5, 3.13595854332114d4,    4.35061725080755d2/
      data (a2(i),i=1,12)/-4.46620341924942d-15, -1.58654991146236d-12, -4.44467627042232d-10, -6.84738791621745d-8, &
                          -6.64932238528105d-6,  -3.69976170193942d-4, -1.12295393687006d-2,  -1.60926102124442d-1, &
                          -8.52408612877447d-1,  -7.45519953763928d-1, 2.98435207466372d0,    1.0d0/
      data (b2(i),i=1,12)/-2.23310170962369d-15, -7.94193282071464d-13, -2.22564376956228d-10, -3.43299431079845d-8, &
                         -3.33919612678907d-6,  -1.86432212187088d-4, -5.69764436880529d-3,  -8.34904593067194d-2, &
                         -4.78770844009440d-1,  -4.99759250374148d-1, 1.86795964993052d0,    4.16485970495288d-1/

...
! open the file (use softlinks to input the desired table)
       ...
       open(unit=999,file='../lib/helm_table.dat',status='old')
...
subroutine invert_helm_ed(epsilon_input, rho_input, abar_input, zbar_input, ye_input, temp_input, temp_output, epsilon_output)
      include 'implno.dek'
      include 'const.dek'
      include 'vector_eos.dek'
...
! call eos one more time with the converged value of the density
      ...
      epsilon_output = etot_row(1) / 9.0D20
...
! REAL PARAMETER !
REAL (DP) :: p_in, temp_in, abar_in, zbar_in, ye_in, rho_trial
REAL (DP) :: rho_out

! ASSIGN INPUT !
ptot_Row(1) = p_in * 6.171D17 * 9.0D20
temp_row(1) = temp_in * 1.0D9
abar_row(1) = abar_in
zbar_row(1) = MIN(zbar_in, ye_in * abar_in)
den_row(1) = rho_trial * 6.171D17
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
SUBROUTINE HELMEOS_RtoP(rho_in, temp_in, abar_in, zbar_in, ye_in, p_out, dpdrho_out, dpdepsilon_out)
USE DEFINITION
IMPLICIT NONE             
INCLUDE 'vector_eos.dek'

! Real parameter !
REAL (DP) :: rho_in, temp_in, abar_in, zbar_in, ye_in             
REAL (DP) :: p_out, dpdrho_out, dpdepsilon_out
...
! assign output !
p_out = ptot_row(1) / 6.171D17 / 9.0D20
dpdrho_out = dpd_row(1) / 9.0D20
dpdepsilon_out = dpT_row(1) / deT_row(1) / 6.171D17
...
! Real parameter !
REAL (DP) :: rho_in, temp_in, abar_in, zbar_in, ye_in
REAL (DP) :: epsilon_out, depsilondT_out
...
! ! assign output !
epsilon_out = etot_row(1) / 9.0D20
depsilondT_out = deT_row(1) / 9.0D20 * 1.0D9
...
DO k = 1, length_step_z_2
	DO j = 1, length_step_r_2
		IF(rho2(j,k) /= rho2_a) THEN
			epsilon_old = epsilon2(j,k)
			temp_old = temp2(j,k)
			CALL invert_helm_ed(epsilon_old, rho2(j,k), abar2(j,k), zbar2(j,k), ye2(j,k), temp_old, temp_new, epsilon_new)
			temp2(j,k) = temp_new

			! I muted this to prevent energy pumbing into system !
			epsilon2(j,k) = epsilon_new
			IF(temp2(j,k) < temp_min) THEN
				temp2(j,k) = temp_min
				CALL HELMEOS_RTOE(rho2(j,k), temp2(j,k), abar2(j,k), zbar2(j,k), ye2(j,k), epsilon_new, dummy)
				epsilon2(j,k) = epsilon_new
			ELSEIF(temp2(j,k) > temp_max) THEN
				temp2(j,k) = temp_max
				CALL HELMEOS_RTOE(rho2(j,k), temp2(j,k), abar2(j,k), zbar2(j,k), ye2(j,k), epsilon_new, dummy)                             
				epsilon2(j,k) = epsilon_new
			ENDIF
...
! assign input !
den_row(1) = rho2(j_in, k_in) * 6.171D17
temp_row(1) = temp2(j_in, k_in) * 1.0D9
abar_row(1) = abar2(j_in, k_in) 
zbar_row(1) = MIN(zbar2(j_in, k_in), ye2(j_in, k_in) * abar2(j_in, k_in))
...
!cs_out = sqrt(gam1_row(1)*p2(j_in, k_in)/rho2(j_in, k_in))
